/*Mikaela Diaz
  9/05/23
  Description: Website code from an ESP32 server to display DHT11, DS18B20, LM35 readings. These readings are displayed in tables.
  The website also displays the sensor readings on ThingSpeak graphs. 
  This code also incorporates the button function so when the button is pressed, a request is sent to get the GPS location of the SIM808.
        ...
        if (c == '\n') {                    // if the byte is a newline character
          ...
          if (currentLine.length() == 0) {
            ...
            if (header.indexOf("GET /on") >= 0) {
              Serial.println("Button Pressend, Sending SMS message");
              float lat, lon;
